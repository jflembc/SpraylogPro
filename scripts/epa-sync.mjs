...
async function loadRowsFromXlsx(buffer) {
  ...
  if (!ws) throw new Error("Could not find EPA worksheet tab.");
...
async function run() {
  const manual = process.env.MANUAL_RUN === "1" || process.argv.includes("--manual");
  const ny = nyParts();

  if (!manual) {
    const isMonday = ny.weekday === "Mon";
    const inSevenPmHour = ny.hour === 19;
    if (!isMonday || !inSevenPmHour) {
      console.log("Skipping: outside Monday 7PM ET window.");
      return;
    }
  } else {
    console.log("Manual run - bypassing time gate");
  }
  ...
  const { data: run, error: runErr } = await sb
    .from("chemical_sync_runs")
    .insert({
      run_type: manual ? "manual" : "scheduled",
      week_key: weekKey,
      status: "running",
      message: "GitHub sync start",
    })
    .select("id")
    .single();
  ...
  try {
    ...
    if (!res.ok) throw new Error(`EPA dump fetch failed: ${res.status}`);

    const buf = Buffer.from(await res.arrayBuffer());
    console.log(`Downloaded ${(buf.length / 1024 / 1024).toFixed(1)} MB`);

    console.log("Parsing XLSX with exceljs...");
    ...
    for (let i = 0; i < mapped.length; i += 500) {
      ...
      if (i % 5000 === 0) console.log(`Upserted ${upserted}...`);
    }
    ...
    console.log(`Done. fetched=${rows.length}, filtered=${filtered.length}, upserted=${upserted}`);
  } catch (e) {
    await sb
      .from("chemical_sync_runs")
      .update({
